...
function Catlab.Graphics.to_graphviz_property_graph(d::AbstractNamedDecapode; typename="", directed=true, kw...)
...
# TODO: generalize ok??
function Catlab.Graphics.to_graphviz_property_graph(d::SummationDecapode; typename="", directed = true, prog = "dot", node_attrs=Dict(), edge_attrs=Dict(), graph_attrs=Dict(), node_labels = true, kw...)
    ...
    map(parts(d, :Op2)) do op
      s, t = d[op, :proj1], d[op, :proj2]
      r = d[op, :res]
      v = add_vertex!(G, label="$(typename(d, s))×$(typename(d,t))", shape="rectangle")
